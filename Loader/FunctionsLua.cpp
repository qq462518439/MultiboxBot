...
int FunctionsLua::GetTradingSkill(std::string name) {
	...
	for (int i = 0; i < numSkill; i++) {
		...
		int r = GetIntFromChar((char*)Functions::GetText("r"));
		std::string n = (char*)Functions::GetText("n");
		if (!h && m > 74 && name == n) return r;
	}
	return 0;
}
...
int FunctionsLua::GetItemCount(std::string item_info) {
...
int FunctionsLua::HasItem(int* item_ids, int size) {
...
int FunctionsLua::GetItemQuality(int bag, int slot) {
...
void FunctionsLua::PickupItem(std::string item_info) {
	for (int i = 0; i <= 4; i++) {
		for (int y = 1; y <= GetContainerNumSlots(i); y++) {
			std::string item_link = GetContainerItemLink(i, y);
			if (item_link.find(item_info) != std::string::npos) {
...
void FunctionsLua::PickupItem(int item_id) {
	for (int i = 0; i <= 4; i++) {
		for (int y = 1; y <= GetContainerNumSlots(i); y++) {
			std::string item_link = GetContainerItemLink(i, y);
			int link_nbr = GetIntFromChar(item_link.c_str());
...
void FunctionsLua::PlaceItem(int slot, std::string itemName) {
...
void FunctionsLua::UseItem(std::string item_info) {
...
WoWUnit* FunctionsLua::GetGroupDispel(std::string dispellType1, std::string dispellType2, std::string dispellType3) {
	...
	for (int i = 1; i <= NumGroupMembers; i++) {
		if ((GroupMember[i] != NULL) && (GroupMember[i]->unitReaction > Neutral)
			...
			&& !Functions::Intersect(localPlayer->position, GroupMember[i]->position, 2.00f)
...
std::tuple<int, int> FunctionsLua::GetSpellID(std::string spell_name) {
	//Execution = 1ms
	int id = 0; int rank = 0;
