	...
	if ((localPlayer->castInfo == 0 || localPlayer->isCasting(RaptorStrikeIDs, 8)) && ((localPlayer->channelInfo == 0) || (localPlayer->flags & UNIT_FLAG_FEIGN_DEATH)) && !localPlayer->isdead && !passiveGroup) {
		ThreadSynchronizer::RunOnMainThread([=]() {
			...
			if (!FunctionsLua::HasPetUI() && FunctionsLua::IsSpellReady("Call Pet")) {
				...
				else if ((localPlayer->speed == 0) && (Moving == 0 || Moving == 4) && (cluster_unit >= 4) && FunctionsLua::IsSpellReady("Volley")) {
					//Volley
				...
				else if ((localPlayer->speed == 0) && (Moving == 0 || Moving == 4) && ((autoShotInRange && !targetPlayer) || ((distTarget > 20.0f) && targetPlayer)) && FunctionsLua::UnitIsElite("target") && FunctionsLua::IsSpellReady("Rapid Fire")) {
					//Rapid Fire
				...
				else if (IsFacing && ((autoShotInRange && !targetPlayer) || ((distTarget > 20.0f) && targetPlayer)) && (localPlayer->speed == 0) && (Moving == 0 || Moving == 4) && FunctionsLua::IsSpellReady("Aimed Shot")) {
					//Aimed Shot
					FunctionsLua::CastSpellByName("Aimed Shot");
				}
				else if (IsFacing && autoShotInRange && (localPlayer->speed == 0) && (Moving == 0 || Moving == 4) && FunctionsLua::IsSpellReady("Multi-Shot")) {
					//Multi-Shot
