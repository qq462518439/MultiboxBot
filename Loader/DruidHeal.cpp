...
static void DruidAttack() {
	...
	if (targetUnit == NULL || targetUnit->isdead || !targetUnit->attackable) {
		...
		else if ((localPlayer->speed == 0) && (Moving == 0 || Moving == 4) && targetPlayer && (EntanglingRootsTimer == 0) && FunctionsLua::IsSpellReady("Entangling Roots")) {
...
static int HealGroup(unsigned int indexP) { //Heal Players and Npcs
	...
	else if ((AoEHeal >= 4) && (distAlly < 40.0f) && FunctionsLua::IsSpellReady("Tranquility")) {
		//Tranquility
...
void ListAI::DruidHeal() {
	...
	if ((ListUnits.size() > LastTarget) && ((localPlayer->isCasting(HealingTouchIDs, 11) && (ListUnits[LastTarget].prctHP > 80))
		...
		ThreadSynchronizer::RunOnMainThread([=]() {
			...
			if ((localPlayer->speed == 0) && (Moving == 0 || Moving == 4) && FunctionsLua::IsSpellReady("Rebirth") && (deadPlayer != NULL)) {
				//Rebirth
